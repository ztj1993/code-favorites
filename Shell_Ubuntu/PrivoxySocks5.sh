...
if [ $? -ne 0 ]; then
    echo "listen-address 0.0.0.0:8118" | tee -a /etc/privoxy/config
else
...
if [ $? -ne 0 ]; then
    echo "forward-socks5 / ${socks5} ." | tee -a /etc/privoxy/config
else
    sed -i "s@^forward-socks5.*@forward-socks5 / ${socks5} .@" /etc/privoxy/config
fi
